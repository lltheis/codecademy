...
function updateComment(url, request) {
  const id = Number(url.split('/').filter(segment => segment)[1]);
  const articleComment = database.comments[id];
  const requestComment = request.body && request.body.comment;
  const response = {};

  if (!id || !requestComment) {
    response.status = 400;
  } else if (!articleComment) {
    response.status = 404;
  } else {
    articleComment.body = requestComment.body || articleComment.body;
    response.body = {comment: articleComment};
    response.status = 200;
  }
...
function deleteComment(url, request) {
  ...
  if (articleComment) {
    database.comments[id] = null;
      const userCommentIds = database.users[articleComment.username].commentIds;
      userCommentIds.splice(userCommentIds.indexOf(id), 1);
...
function upvoteComment(url, request) {
  ...
  const username = request.body && request.body.username;
  let articleComment = database.comments[id];
  const response = {};

  if (articleComment && database.users[username]) {
    savedArticle = upvote(articleComment, username);
    response.status = 200;
    response.body = {comment: articleComment};
    return response;
  } else {
...
function downvoteComment(url, request) {
  ...
  if (articleComment && database.users[username]) {
    savedArticle = downvote(articleComment, username);
    response.status = 200;
    response.body = {comment: articleComment};
    return response;
  } else {
